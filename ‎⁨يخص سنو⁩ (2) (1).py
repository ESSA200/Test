...
MAX_EMAILS = 6
TOKEN = '6753540305:AAETaqY5GXlbIwpigoSn_4JzvVN_xYC_xkE'
bot = telebot.TeleBot(TOKEN)

def load_users():
    if os.path.exists('users5.json'):
...
def load_emails():
    if os.path.exists('email11.json'):
        with open('email11.json', 'r') as f:
            data = json.load(f)
            for user_id in data:
...
def save_emails(data):
    with open('email11.json', 'w') as f:
        json.dump(data, f, indent=4)

@bot.message_handler(commands=['start'])
def send_welcome(message):
    users = load_users()
    if str(message.from_user.id) in users or message.from_user.id == 503071055:
    ...
    else:bot.send_message(message.chat.id, "عذراًلم تقم بالاشتراك بالبوت! ، للاشتراك تواصل مع @KKNTW")
...
def handle_id(message):
    if message.from_user.id == 503071055:
